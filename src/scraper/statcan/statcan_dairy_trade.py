...
def scrape(db_params):
    try:
        ...
        date_from = dbm.get_latest_date_record('select max(yearmonth) from statcan_dairy_trade;')
        ...
        #year_from = datetime.strptime('2000','%Y').year
        #month_from = datetime.strptime('01','%m').month
            
        today = datetime.strftime(datetime.now(), '%Y_%m_%d')
        dir_path = create_directory(WEB_STATCAN_PATH+'dairy_trade\\', today)
        ...
        temp_url = ('http://www5.statcan.gc.ca/cimt-cicm/topNCountryCommodities-marchandises?'
                    ...
                    'Milk%2C+not+concentrated+and+unsweetened%2C+not+exceeding+1%25+fat&lang=eng'
                    '&refYr=2014&sectionId=&chapterId=4&arrayId=9800000&provId=1&refMonth=12&commodityId=40110')
        ...
        UserAgents = ['Mozilla/5.0 (Windows NT 6.2; rv:22.0) Gecko/20130405 Firefox/22.0','Mozilla/5.0 (Windows NT 6.2; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1667.0 Safari/537.36','Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0; SLCC2; Media Center PC 6.0; InfoPath.3; MS-RTC LM 8; Zune 4.7',
        ...
        for k in range(1,len(hscode)): # ignore first code to avoid duplicate data - DON'T CHANGE THIS
            for year in years: 
                ...
                for month in months: 
                    ...
                    url = ('http://www5.statcan.gc.ca/cimt-cicm/topNCountryCommodities-marchandises?countryId=117&'
                        'tradeType=1&usaState=0&topNDefault=250&freq=6&commodityName=Milk%2C+not+concentrated+and+'
                        'unsweetened%2C+not+exceeding+1%25+fat&lang=eng&refYr=' + str(year) + '&sectionId=1&monthStr='
                        'August&chapterId=4&arrayId=9800000&sectionLabel=I+-+Live+animals+and+animal+products.&provId='
                        '1&refMonth=' + str(month) +'&commodityId=' +hscode[k][:6])
                    #url='http://www5.statcan.gc.ca/cimt-cicm/topNCountries-pays?lang=eng&sectionId=1&dataTransformation=0&refYr=' + 
                    #'year[i] + '&refMonth=' + month[j] + '&freq=6&countryId=0&usaState=0&provId=1&retrieve=Retrieve&save=null&country'
                    #'=null&tradeType=1&topNDefault=250&monthStr=null&chapterId=2&arrayId=0&sectionLabel=I%20-%20Live%20animals%20and%20'
                    #'animal%20products.&scaleValue=0&scaleQuantity=0&commodityId=' + hscode[k][0]
                    test=False
                    x=0
        ...
        output.to_csv(dir_path+'statcan_transformed_results.csv')
        print 'Finished scraping at %s.' % datetime.now()
    except Exception as err:
        exc_info = sys.exc_info()
        error_msg = 'auto_run() scrape error:\n'
        msg_list = [['statcan_dairy_trade.py'],[error_msg], [str(exc_info[0])], [str(exc_info[1])], [str(exc_info[2]) + '\n']]
        print msg_list
        save_error_to_log('monthly', msg_list)
    else:
        success_msg = 'auto_run() scraped successfully\n'
        msg_list = [['statcan_dairy_trade.py'],[success_msg]]
        save_error_to_log('monthly', msg_list)
