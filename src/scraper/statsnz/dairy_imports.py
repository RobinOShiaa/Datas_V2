...
def scrape(db_params):
    try:
        ...
        sql = ('select max(yearmonth) from statsnz_dairy_trade where flow="import";')
        ...
        # click Go button to skip ahead for yearmonths
        ...
        dir_path = WEB_STATSNZ_PATH + 'statsnz_dairy_trade\\'
        ...
        for yearmonth in yearmonths:
            for geo_group in geo_groups:
                ...
                try:
                    ...
                    pass
                # select Data type: Imports
                wscraper.click_button('id', 'ctl00_MainContent_rbTIM')
                ...
                wscraper.load_field('input', 'name', 'hsCodeSearchText', '040')
                wscraper.click_button('xpath', '//td/div/a[@href="javascript:OnHSCodeSearch();"]')
                time.sleep(4)
                
                # click checkboxes starting with '040'
                wscraper.click_checkbox_by_xpath('//tr/td/div/div/input[starts-with(@value, "0401")]')
                wscraper.click_checkbox_by_xpath('//tr/td/div/div/input[starts-with(@value, "0402")]')
                wscraper.click_checkbox_by_xpath('//tr/td/div/div/input[starts-with(@value, "0403")]')
                wscraper.click_checkbox_by_xpath('//tr/td/div/div/input[starts-with(@value, "0404")]')
                wscraper.click_checkbox_by_xpath('//tr/td/div/div/input[starts-with(@value, "0405")]')
                wscraper.click_checkbox_by_xpath('//tr/td/div/div/input[starts-with(@value, "0406")]')
                ...
                #action.key_up(Keys.RETURN).perform()
                time.sleep(5)
        ...
        msg_list = [['statsnz.dairy_trade.py'],[error_msg], [str(exc_info[0])], [str(exc_info[1])], [str(exc_info[2]) + '\n']]
        print msg_list
        save_error_to_log('monthly', msg_list)
    else:
        success_msg = 'auto_run() scraped successfully\n'
        msg_list = [['statsnz.dairy_trade.py'],[success_msg]]
        save_error_to_log('monthly', msg_list)
