...
def scrape(db_params):
    ...
    for url in urls[1:]:
        ...
        urls_underline = urls_underline[:-1] #takes special offers off list
        ...
        for url_2 in urls_underline:
            ...
            for url_3 in menu_aisles:
                ...
                for url_4 in menu_shelf:
                    ...
                    title = browser.find_element('xpath', '//*[@id="pageTitle"]/h1').text.replace('/','-') # 50/50 bread
                    ...
                    product_cells = soup.find_all('div',{'class':'listings'})[0].find_all('div',{'class':'container'})
                    for p_c in product_cells:
                        try:
                            product_tag = p_c.find('span',{'id':'productTitle'}).text
                        except:
                            print products
                            continue
                        #print product_tag
                        products.append(product_tag.encode('ascii','replace'))
                        
                        try:
                            unit_tag = p_c.find('p',{'class':'subTitle'}).text
                        except:
                            unit_tag = ''
                        units.append(unit_tag)
                        
                        price_tag = p_c.find('span',{'class':'price'}).text
                        prices.append(price_tag.encode('ascii','replace'))
                        
                        try:
                            ppu_tag = p_c.find('span',{'class':'priceInformation'}).text
                        except:
                            ppu_tag = 'N/A'

                        ppu.append(ppu_tag.encode('ascii','replace'))
                    
#                     product_tags = browser.find_elements('xpath','(//div[@id="listings"])[1]//div[@class="product-content"]//span[@id="productTitle"]')
#                     print len(product_tags)
#                     unit_tags = browser.find_elements('xpath','(//div[@id="listings"])[1]//p[@class="subTitle"]')
#                     print len(unit_tags)
#                     price_tags = browser.find_elements('xpath','(//div[@id="listings"])[1]//div[@class="price-cart-block"]/div/span[@class="price"]')
#                     print len(price_tags)
#                     ppu_tags = browser.find_elements('xpath','(//div[@id="listings"])[1]//div[@class="price-cart-block"]/div/span[@class="priceInformation"]')
#                     print len(ppu_tags)
#                     for p in product_tags:
#                         products.append(p.text.encode('ascii','replace'))
#                     
#                     for p in unit_tags:
#                         units.append(p.text.encode('ascii','replace'))
#                     
#                     for p in price_tags:
#                         prices.append(p.text.encode('ascii','replace'))
#                         
#                     for p in ppu_tags:
#                         ppu.append(p.text.encode('ascii','replace'))
                    ...
                    with open(file_path, 'wb') as out_file:
                        ...
                        for i in range(0,len(products)):
                            out_file.write(products[i]+' '+units[i]+','+prices[i]+','+ppu[i]+'\n')
                    browser.close()        
