...
def scrape():
    ...
    #     for h in headers:
    #         #print h.text
    #         if h.text not in ['Options','Charts','Month']:
    #             data.append(h.text.replace('\n', ' '))
        ...
        #print data
        data = chunck_list(data, 10)#len(headers)-2)
        
        dir_path = WEB_CME_PATH + 'live_cattle/'
    ...
    #             file.write('\n')
        ...
        with open ('Y:\\SITES\\node\\public\\current_cattle.csv', 'wb') as file:
            file.write('Product,'+','.join(data[0])+'\n')
             
            for w in data[1:3]:
                file.write('Live cattle futures,' + ','.join(w) + '\n')
        ...
        msg_list = [['live_cattle_futures.py'],[error_msg], [str(exc_info[0])], [str(exc_info[1])], [str(exc_info[2]) + '\n']]
        print msg_list
        save_error_to_log('daily', msg_list)
    else:
        success_msg = 'auto_run() loaded successfully\n'
        msg_list = [['live_cattle_futures.py'],[success_msg]]
        save_error_to_log('daily', msg_list)
