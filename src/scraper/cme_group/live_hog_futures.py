...
def scrape():
    try:
        ...
        #data = []
        url = 'http://www.cmegroup.com/trading/agricultural/livestock/lean-hogs.html'
    ...
    #             data.append(h.text.replace('\n', ' '))
        ...
        dir_path = WEB_CME_PATH + 'live_hogs/'
    ...
    #             file.write('\n')
        ...
        with open ('Y:\\SITES\\node\\public\\current_pig.csv', 'wb') as file:
            file.write('Product,'+','.join(data[0])+'\n')
            
            for w in data[1:3]:
                file.write('Lean hog futures,' + ','.join(w) + '\n')
        ...
        msg_list = [['live_hog_futures.py'],[error_msg], [str(exc_info[0])], [str(exc_info[1])], [str(exc_info[2]) + '\n']]
        print msg_list
        save_error_to_log('daily', msg_list)
    else:
        success_msg = 'auto_run() loaded successfully\n'
        msg_list = [['live_hog_futures.py'],[success_msg]]
        save_error_to_log('daily', msg_list)
