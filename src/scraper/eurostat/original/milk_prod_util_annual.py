...
LOG_PATH = EUROSTAT_PATH + 'milk_prod_util_annual.log'
PROGRAM_PATH = './src/scraper/eurostat/milk_prod_util_annual.py'
LABEL_PATH = EUROSTAT_PATH + 'milk_prod_util_annual/title_options/'
...
def scraping(web_driver, prodmilk_point, time_point, milkitem_point,
    ...
    for id in time_id:
        web_scraper.click_button('//input[@id="%s"]' % (id), 'xpath')
    web_scraper.click_button('//input[@id="%s"]' % (time_id[-1]), 'xpath')
    ...
    for prodmilk in PRODMILK_LABELS:
        web_scraper.load_field("model.projDimsValuesAvailable['DS-052668PRODMILK']", prodmilk[1], 'list')
        time.sleep(3)
        
        for milkitem in MILKITEM_LABELS:
            web_scraper.load_field("model.projDimsValuesAvailable['DS-052668MILKITEM']", milkitem[1], 'list')
            time.sleep(5)
...
if __name__ == '__main__':
    ...
    dir_path = EUROSTAT_PATH + 'milk_prod_util_annual/'
