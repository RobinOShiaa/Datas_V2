...
05/02/2015(Wenchong): Added recursion to resume from break point.
06/02/2015(Wenchong): Fixed bugs.
...
def recover_scrape(partner_brk_point=0, time_brk_point=0):
    global NUM_PART
    global PARTNER_IDS
    global TIME_IDS
    
    file_name = 'pig_meat_trade_hs6'
    
    # this bookmark_url selects all flows, indicators, HS6 products and reporters, partner ZZ and period 198801 selected
    bookmark_url = 'http://appsso.eurostat.ec.europa.eu/nui/show.do?query=BOOKMARK_DS-016893_QID_-1E283317_UID_-3F171EB0&layout=PERIOD,L,X,0;REPORTER,L,Y,0;PARTNER,L,Z,0;PRODUCT,L,Z,1;FLOW,L,Z,2;INDICATORS,C,Z,3;&zSelection=DS-016893PARTNER,US;DS-016893INDICATORS,VALUE_IN_EUROS;DS-016893FLOW,1;DS-016893PRODUCT,TOTAL;&rankName1=PARTNER_1_2_-1_2&rankName2=INDICATORS_1_2_-1_2&rankName3=FLOW_1_2_-1_2&rankName4=PRODUCT_1_2_-1_2&rankName5=PERIOD_1_0_0_0&rankName6=REPORTER_1_2_0_1&sortC=ASC_-1_FIRST&rStp=&cStp=&rDCh=&cDCh=&rDM=true&cDM=true&footnes=false&empty=false&wai=false&time_mode=NONE&time_most_recent=false&lang=EN&cfo=%23%23%23%2C%23%23%23.%23%23%23'
    ...
    for partner_id in tmp_partner_ids:
        ...
        for time_id in tmp_time_ids:
            try:
                ...
                wscraper.download_eurostat_file(file_name, bookmark_url, 80, part)
            except Exception, e:
                ...
                recover_scrape(PARTNER_IDS.index(partner_id), TIME_IDS.index(time_id))
                return
            else:
                NUM_PART += 1
                wscraper.close()
...
def scrape(db_params):
    ...
    sql = ('select max(yearmonth) as max_date from eurostat_pig_meat_trade_hs6 '
           'group by reporter, partner, product_code, flow order by max_date asc limit 1;')
