...
def scrape(db_params):
    try:
        ...
        wscraper.download_eurostat_file(file_name, bookmark_url, 40, '')
        ...
        msg_list = [['eurostat.other_slaughtering'],[error_msg], [str(exc_info[0])], [str(exc_info[1])], [str(exc_info[2]) + '\n']]
        print msg_list
        save_error_to_log('monthly', msg_list)
    else:
        success_msg = 'auto_run() scraped successfully\n'
        msg_list = [['eurostat.other_slaughtering'],[success_msg]]
        save_error_to_log('monthly', msg_list)
